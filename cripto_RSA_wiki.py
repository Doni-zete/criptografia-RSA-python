...
# Alguns dos métodos em java não existiam em python e porisso foram implementados.
...
# As variaveis  "p" e "q" estão recebendo doi valores primos. Esses valores são desse tamanho porque 
...
# Aqui foi implementado um método que descobre o maior divisor comum entre doi números.
...
print("n: {}".format(n)) # Utilizado em conjunto com as chaves publica e privada 
print("e: {}".format(e)) # privado
print("d: {}".format(d)) # publica
...
def mensagemCigrada():
    ...
    # Aqui foi está utilizando o método proprio do python para fazer a exponenciação de um numero e 
    # tirar o mod do valor. Como os valores eram estremamente grandes não foi possivel utilizar um método implmentado por conta propria.
...
def mensagemDecifrada():
    ...
    # Exemplo (01000001 -> 65) 
    arraDeBits = [int(binaryFromDecimal[i*8:8*(i+1)], 2) for i, _ in enumerate(range(int(len(binaryFromDecimal)/8)))]
    
    # Aqui está exibindo cada valor referente da tabela ASCII
    print("Valores da tabela ASCII:", arraDeBits)

    # Aqui está convertendo cada valor numerico da tabela ASCII para sua forma de caracteres.
